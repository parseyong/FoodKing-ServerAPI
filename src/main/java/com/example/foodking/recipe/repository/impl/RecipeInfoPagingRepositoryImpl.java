...
public class RecipeInfoPagingRepositoryImpl implements RecipeInfoPagingRepository {
    ...
    @Override
    public List<ReadRecipeInfoResDTO> findRecipeInfoPaging(Pageable pageable, OrderSpecifier[] createOrderSpecifier, BooleanBuilder builder) {
        List<Tuple> result = jpaQueryFactory.select(recipeInfo,recipeEmotion.count())
                .from(recipeInfo)
                .leftJoin(recipeEmotion)
                .on(recipeInfo.recipeInfoId.eq(recipeEmotion.recipeInfo.recipeInfoId))
                .where(builder)
                .groupBy(recipeInfo)
